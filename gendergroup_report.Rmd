...
The default option and associated risks remained the same in the updated version. The only change was that of design to match the design of the new decision options 2 pathway \@ref(fig:defaultchunk).
...
```
Visulised is the Cashflow  for the first option, a private insurance payed by having an own business branch using the plot_cashflow() function with the mcSimulation() outputs. 
...
In general, it can be said that the investment into ETFs and a mix with private insurance were the best investments in most cases with some expections. For instance, way 2 showed more negative values than the default option. As a conclusion it can be said that a monitoring plan seems to be recommendable.

Notes from Julia: I left this out

Other ways with EVPI:11 and 2,too!!!!
