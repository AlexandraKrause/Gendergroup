...
```
Using a Partial least square regression, a VIP score, Variable importance of a projection was produced.It shows which variables have the strongest impact on the outcome but also the correlation with the outcome. More decision analysis in research? Should we do more moitoring? The VIP shows the importance of the variables with a correlation of the output. The highest influences on the output are shown in the plot. Positive and negative values are visualised in comparison to the baseline option, showing if their influence is more positive or negative than the baseline
Bar graphs (right) indicate variable importance
(expressed by VIP scores) for variables to which the model was most
sensitive (green bars indicate positive relationships with the outcome
variables, red bars indicate negative relationships
...
To guide further research, the EVPI analysis, an analysis of expected value of perfect information was calculated. The EVPI represents the amounts of money, a decision maker should be willing to pay to reduce the uncertainty concerning the respective variable. The variables with the highest uncertainty are shown. The EVPI shows where it would be suggested to invest more money to get better information about one variable. For broader ranges of lower and upper values, or rather more insecurity in the calculation of one variable the EVPI is positive. Further research could observe these variables to better the models.The x-axis is in euro.
Most options had no variables with a positive EVPI. The expected value of information (EVPI) was calculated for the decision (Figure 7). 
...
For number 7 this mix shows a positive EVPI over .... euro, which measn that further research on this variable should be done. Other ways with EVPI: 11,12,..?still running, might have missed s.th.-> 11 and 2,too!!!!
