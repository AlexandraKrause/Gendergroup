...
input_table_gender <-read.csv2("./input_table_gender_final_trial_years_woRisk_Test1Way.csv", dec = ",")
...
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_results, 
                                    vars = c("NPV_profit_with_off_farm_job_6"),
                                    method = 'smooth_simple_overlay', 
                                    base_size = 7)

decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_results, 
                                    vars = c("NPV_decision_profit_with_off_farm_job_6"),
                                    method = 'smooth_simple_overlay', 
                                    base_size = 7)
...
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_results, #without _wayx , this code wil not work.
                                    vars = c("NPV_no_branch",
                                             "NPV_profit_with_Own_business_branch_6"),
                                    method = 'boxplot')
...
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_results, #without _wayx , this code wil not work.
                                    vars = "NPV_profit_with_Own_business_branch_6",
...
pls_result <- plsr.mcSimulation(object = mcSimulation_results,
                                variables.x = names(mcSimulation_results$x),
                                resultName = names(mcSimulation_results$y)[c(3)], ncomp = 1)

#pls_result <- plsr.mcSimulation(object = mcSimulation_results,
#                                resultName = names(mcSimulation_results$y)[c(62)], ncomp = 1)

#pls_result <- plsr.mcSimulation(object = mcSimulation_results,
#                                resultName = names(mcSimulation_results$y)[c(121)], ncomp = 1)

mcSimulation_results
colnames(mcSimulation_results$y)[3]
...
evpi <- multi_EVPI(mc = mcSimulation_table, first_out_var = "NPV_decision_profit_withOwn_business_branch_1")
